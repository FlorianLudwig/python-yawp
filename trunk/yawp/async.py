...
Makes the wsgi werkzeug debugger usable with tornado
"""
import sys
import urllib
import StringIO
...
class DebugRequestHandler(tornado.web.RequestHandler):
    ...
    def get_error_html(self, status_code, **kwargs):
        if 'exception' in kwargs:
            return self.application.get_error_html(self, status_code, **kwargs)
        else:
...
class DebugApplication(tornado.web.Application):
    ...
    def  __call__(self, request):
        if request.path.rstrip('/').endswith('/__debugger__'):
            print 'wsgi handler'
